(function (global) {
     ...
     */
    module.exports.NUnitXmlReporter = function (options) {
        var self = this;
        self.started = false;
        self.finished = false;
        // sanitize arguments
        ...
        self.jasmineDone = function () {
            ...
            log(totalSpecsExecuted + " specs, " + totalSpecsFailed + " failures");
        };
    ...
    function specAsXml(spec, indent) {
        ...
        if (isFailed(spec)) {
            var totalExpectationReported = spec.failedExpectations.length > 1 ? 1 : 0;
            totalSpecsReported++;
            spec.failedExpectations.forEach(function (expectation) {
                var currentExpectation = totalExpectationReported ? "." + totalExpectationReported++ : "";
                log(totalSpecsReported + currentExpectation + ") " + spec.fullName);
                log("  Message:");
                log("    " + expectation.message);
                log("  Stack:");
                log("    " + expectation.stack);
                log("");
            });
        }
